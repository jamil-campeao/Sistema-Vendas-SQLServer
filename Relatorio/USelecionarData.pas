...
begin
 if (Edt_datafinal.Date) < (edt_datainicio.Date) then
  ...
  if (edt_datafinal.Date=0) Or (edt_datainicio.Date=0) then
    begin
    MessageDlg('Data inicial ou data final são campos obrigatórios',mtinformation,[mbok],0);
...
{  if (edt_datainicio.Date > Date ) then
    begin
    MessageDlg('Data inicial não pode ser maior que hoje',mtinformation,[mbok],0);
    edt_datainicio.SetFocus;
    abort;
    end;
    }
